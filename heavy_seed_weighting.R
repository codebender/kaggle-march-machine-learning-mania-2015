...
matches$calculated <- 0.50 + (matches$highIdSeed - matches$lowIdSeed)*0.03333333333
submit <- data.frame(id = matches$id, pred = matches$calculated)
write.csv(submit, file = "Output/heavy_simple_seed.csv", row.names = FALSE, quote=FALSE)
